...
def main(host='localhost', port=8086):
    ...
    try:
        for df in pd.DataFrame.from_dict(res, orient='index'):
            df.to_sql(name="testdb8", con=engine, if_exists='append', method="multi", chunksize=1000)
    except Exception as e:
        print("Error : ", e)
        sys.exit()
    ...
    # df.to_sql(name="testdb7", con=engine, if_exists='append', method="multi", chunksize=1000)
